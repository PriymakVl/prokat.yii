...
<div class="content">
    ...
    <? else: ?>
        <div class="title-box">Перечень оборудование по участкам</div>
    <? endif; ?>
    ...
    <table class="margin-top-15">
        ...
        <? if ($items): ?>
            <? foreach ($items as $item): ?>
                <tr>
                    ...
                    <td>
                        <a href="<?=Url::to(['/equipment/list', 'parent_id' => $item->id])?>"><?=$item->name?></a>
                    </td>
                    <td><?=$item->alias?></td>
        ...
        <? else: ?>
            <tr>
                <td colspan="3" class="not-content">Оборудования еще нет</td>
            </tr>
        <? endif; ?>
    </table>
</div><!-- class content -->
